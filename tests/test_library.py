...
class Test_get_all_library_paths:
    ...
    def test_should_hash_correctly(library, paths, expected_library_paths):
        ...
        assert all([str(p).startswith("20") for p in paths])  # Should be relative path
        assert paths == [Path(p) for p in expected_library_paths]


class Test_get_all_incoming_paths:
    @staticmethod
    @pytest.mark.parametrize("paths,expected_library_paths", [
        (["./incoming/IMG_0011.jpg", "./2020/05/2.jpg", "./other/2020/05/2.jpg"], ["./incoming/IMG_0011.jpg"]),
    ], ids=[
